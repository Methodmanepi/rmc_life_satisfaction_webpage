...
author: "METHODIUS OKOUZI" # Remember to add your name!
date: "24-07-2024"
theme: united
---
...
pacman::p_load(plotly, tidyverse, reactable, here, scales, fastmap)
...
# Create table
 ...
 table_life_sat <- cantril %>% 
  filter(Year %in% c(2011, 2023)) %>% 
  pivot_wider(names_from = Year, values_from = `Cantril ladder score`) %>% 
  select(!Code) %>% 
  mutate(`Absolute Change` = round(`2011`-`2023`, 2)) %>% 
  mutate(`Relative Change` = scales::percent(round(`Absolute Change`/`2011`, 2))) %>% 
  reactable()
...
# Create line chart
chart_meth<- cantril %>% 
  filter(Country %in% c("Austria", "Bahrain", "Africa", "Afgahnitian", "Albania", "Algeria", "Nigeria", "Kenya")) %>% 
  ggplot(aes(Year, `Cantril ladder score`, color = Country))+
  geom_line()
